...
POINTS_PER_RING = 40
RING_SIZE = 14.
STRAIN_SCALE = 17. * RING_SIZE # Factor to scale h+ for z-displacement (TUNE THIS!)
...
def generate_ring_cartesian_coords(
    ...
    ring_center_phis = np.linspace(0, 2 * np.pi, num_observatories, endpoint=False)
    ...
    # Reshape array to (num_observatories, num_radii, 1) and (1, 1, 3)
    ...
    x_arms = np.array([[[arm_length, 0, 0]]])
    y_arms = np.array([[[0, arm_length, 0]]])
    z_arms = np.array([[[0, 0, arm_length]]])
...
def create_merger_movie():
    ...
    x_grid, y_grid, z_grid, center_directions, strain_at_centers, x_centers, y_centers, z_centers = generate_ring_data(strain_modes_sxs, NUM_RINGS, RING_SIZE, r_axis)
    ...
    for i_frame, current_lab_time in enumerate(anim_lab_times):
        ...
        mlab.quiver3d(x_centers, y_centers, z_centers, x_grid_for_plotting - x_centers, y_grid_for_plotting - y_centers,
                      z_grid_for_plotting - z_centers, scalars=displacement_scalars,
                      color=(0.95, 0.05, 0.05), colormap='copper', opacity=0.9, scale_mode='vector', scale_factor=1)
        ...
        if i_frame == NUM_FRAMES:
            for extra in range(2*FPS):
                frame_filename = f"{frames_dir_path}/frame_{i_frame:04d}_extra_{extra:03d}.png"
                cv2.imwrite(frame_filename, combined_frame)
                frame_files.append(frame_filename)
