...
MAX_R_GW = 600.0
NUM_PHI_GW = 300
AMPLITUDE_SCALE = 0.40 * MAX_R_GW # Factor to scale h+ for z-displacement (TUNE THIS!)
...
BH_ELEVATION = 10.0 # Set 0 or a positive float to have the BHs rotate above the strain surface
PIP_SCALE = 3.0 # This is the ratio of window to PiP
...
def generate_adaptive_r_coords(
    ...
    """
    # Note to self: You could avoid interpolating waveform data and just base r-steps off sample timesteps.
    # However, I've chosen not to do this because:
    # a) it keeps the script more generalizable by not relying on SXS's habits -
    #    If hard conventions can be found and are similar to BH@H's habits, might be worth reevaluating
    # b) the vectorized interpolation call isn't actually very expensive
    # c) decent amount of work required to adjust related functions

    if old_r_axis is not None:
        temp = np.searchsorted(old_r_axis, r_max - lab_time_step, side='left')
        r_points = old_r_axis[:temp] + lab_time_step
        num_chopped = len(old_r_axis) - temp - 1
        # Avoid an off-by-one error or hiding shape errors that arise from not accounting for appending r_max
...
def reconstruct_hplus_on_xy_plane_at_time_t(
    ...
    """
    num_phis = len(phi_coords)
    if old_z_grid is not None:
        buffer_size = len(r_coords) - len(old_z_grid) + num_points_chopped
        grid_buffer = np.zeros((buffer_size, num_phis))
        z_displacement_grid = np.concatenate((grid_buffer, old_z_grid[:(-num_points_chopped - 1)], np.zeros((1, num_phis))), axis=0)
        color_scalars = np.concatenate((grid_buffer, old_color_scalars[:(-num_points_chopped - 1)], np.zeros((1, num_phis))), axis=0)
        r_coords_for_eval = np.concatenate((r_coords[:buffer_size], [r_coords[-1]])) # The last element of r_coords is the APPENDED r_max
    ...
    eval_list = list(range(len(r_coords_for_eval) - 1)) + [-1] # one of the evaluation points is the very outer edge
    for i in eval_list:
...
def create_merger_movie():
    ...
    peak_h_x_value = 0.9 * np.max(reconstruct_hplus_on_xy_plane_at_time_t(None, None,
        strain_modes_sxs, peak_strain_time, 0, np.asarray([1]), phi_gw_axis)[1][0])
    print(peak_h_x_value)

    # returns a tuple which we slice for just the color_scalars grid. Our input radial axis is one point, so we slice again for the 1D
    ...
    for i_frame, current_lab_time in enumerate(anim_lab_times):
        ...
        # plot GW surface
        temp_time = time.time()
        mlab.mesh(X_GW_SURF, Y_GW_SURF, z_gw_frame, scalars=GW_color_scalars,
                  colormap='winter', representation = representation_str, name="GW h+ Surface",
                  opacity=0.9, vmin = -peak_h_x_value, vmax = peak_h_x_value)
        if timing_bool:
